...
def run(key, func, shape):
    ...
    if "Raft" in key[1] or "CuPy" in key[1]:
        device = cupy.cuda.Device(cupy.cuda.runtime.getDevice())
        preds = cupy.random.uniform(-5., 5., shape, dtype="float32")
        obsrvs = cupy.random.uniform(-5., 5., shape, dtype="float32")
        ...
        kwargs = dict(dtype=torch.float32, device="cuda", requires_grad=False)
        ...
        preds = numpy.random.uniform(-5., 5., shape).astype(numpy.float32)
        obsrvs = numpy.random.uniform(-5., 5., shape).astype(numpy.float32)
...
if __name__ == "__main__":
    ...
    logfile = rootdir.joinpath("results", f"perf_log_v2_float32.csv")
