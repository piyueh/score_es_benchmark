...
template<typename T>
const KERNEL_ARRY_t_c<T> v4_kernels{FACTORY(T, KERNEL_ARRY_t_c, kernel_v4)};

// instantiations: actual kernel holders for different FPs
template const KERNEL_ARRY_t_c<float> v4_kernels<float>;
template const KERNEL_ARRY_t_c<double> v4_kernels<double>;
...
T earth_mover_v4(  // args need .noconvert in pybind11 module init
    ...
    const auto &kernel = v4_kernels<T>.at(ncols-1);  // w/ boundcheck
